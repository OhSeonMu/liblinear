...
// TODO[osm]
model* train_with_port(const problem *prob, const parameter *param, int port)
{
	int i,j;
	int l = prob->l;
	int n = prob->n;
	...
	if(check_regression_model(model_))
	{
		...
		// TODO[osm] 
		int test = 1;
		if (CreateSocket(socket_fd, opt, address, port) < 0) {
		      test = 0;
		}
		...
		while (test) {
			...
			if (std::sscanf(buffer, "%d %d", &is_stop, &not_use) != 2) {
